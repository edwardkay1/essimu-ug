...
const BestSellers = () => {
    ...
    {/* Dynamic Badge (remains on top of image) */}
    {product.badge && (
        <span className={`absolute top-4 left-4 z-10 px-2 py-1 rounded-md text-[10px] font-bold text-white ${
            product.badgeType === 'discount' ? 'bg-red-500' : 'bg-blue-600'
        }`}>
            {product.badge}
        </span>
    )}

    {/* Image component filling to the edges */}
    <Image 
        src={product.image} 
        alt={product.name} 
        fill
    ...
    {/* Optional: Dark gradient overlay for text readability if you have text on image */}
    <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                        ...
                        <div className="p-5 flex flex-col flex-grow">
                            <div className="flex justify-between items-start mb-1">
                                <h3 className="font-bold text-[#111418] dark:text-white leading-tight max-w-[80%]">
                                    {product.name}
                                </h3>
                            ...
                            <p className="text-xs text-[#60758a] dark:text-[#9ca3af] mb-6">
                                {product.specs}
                            </p>
                            ...
                            <div className="mt-auto flex justify-between items-center">
                                ...
                                <button className="p-2 rounded-full border border-blue-100 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition-colors" aria-label="Shop Now">
