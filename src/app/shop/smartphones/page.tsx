...
import { Smartphone, ChevronRight, Activity, SmartphoneNfc, Search, X, Loader2 } from "lucide-react";
...
export default function SmartphonesPage() {
    ...
    // --- LIVE CLOUD SYNC ---
    useEffect(() => {
        // This leverages the "category + createdAt" index you created
        const q = query(
            collection(db, "products"),
            where("category", "==", "Phone"), // Matches your Admin Form value
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            console.error("Smartphone Sync Error:", error);
            setLoading(false);
        });
    ...
    }, [selectedFilters, searchQuery, phones]);
    ...
    return (
        <div className="bg-white dark:bg-[#0a0a0a] min-h-screen">
            ...
            <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-20 py-8 lg:py-12">
                ...
                {/* --- NAVIGATION --- */}
                <nav className="flex items-center gap-3 text-[9px] font-black uppercase tracking-[0.2em] mb-10 overflow-x-auto whitespace-nowrap">
                    <Link href="/" className="text-gray-400 hover:text-[#0070f3]">Home</Link>
                    <ChevronRight size={10} className="text-gray-300" />
                    <span className="text-[#0070f3]">Smartphones</span>
                </nav>
                ...
                <div className="bg-[#0070f3] rounded-[2.5rem] lg:rounded-[3.5rem] p-8 lg:p-16 mb-12 lg:mb-16 text-white relative overflow-hidden flex items-center shadow-2xl">
                    <div className="relative z-10 max-w-xl">
                        ...
                        <h1 className="text-4xl lg:text-7xl font-black mb-4 lg:mb-6 leading-none tracking-tighter uppercase">
                            Mobile <br/> <span className="text-blue-200">Excellence</span>
                        </h1>
                    </div>
                    ...
                    <main className="flex-1">
                        {loading ? (
                            <div className="py-24 flex flex-col items-center justify-center gap-4">
                                ...
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Loading B118 Registry...</p>
                            </div>
                        ) : (
                            <>
                                <div className="flex justify-between items-end mb-10 border-b border-gray-100 dark:border-white/5 pb-6">
                                    <div>
                                        <h2 className="text-2xl lg:text-4xl font-black text-gray-900 dark:text-white tracking-tighter uppercase">Device List</h2>
                                        <p className="text-[9px] font-black text-[#0070f3] uppercase tracking-[0.2em] mt-2">
                                            {filteredItems.length} Units Ready to Ship
                                        </p>
                                    </div>
                                </div>
                                ...
                                {filteredItems.length > 0 ? (
                                    ...
                                    <div className="py-24 flex flex-col items-center text-center bg-gray-50 dark:bg-white/[0.02] rounded-[3rem] border-2 border-dashed border-gray-100 dark:border-white/5">
                                        ...
                                        <p className="text-gray-500 font-black uppercase tracking-widest text-[10px]">No Devices Found</p>
                                    </div>
                                )}
