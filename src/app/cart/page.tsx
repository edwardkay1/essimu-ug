...
export default function CartPage() {
    ...
    const handleWhatsAppCheckout = () => {
        const phone = "256756922058"; // Updated to your ESSIMU Business Line
        let message = `*ESSIMU HARDWARE ACQUISITION*\n`;
        message += `_Ref: Web-Sync Order_\n`;
        message += `--------------------------\n`;
        cart.forEach(item => {
            message += `📦 *${item.name}*\n`;
        ...
        message += `--------------------------\n`;
        message += `*ESTIMATED TOTAL: UGX ${subtotal.toLocaleString()}*\n`;
        message += `_Delivery: KLA Hub B118_`;
            ...
            <main className="max-w-[1440px] mx-auto px-6 lg:px-20 py-16">
                ...
                <div className="flex flex-col gap-2 mb-12">
                    <nav className="flex items-center gap-3 text-[9px] font-black uppercase tracking-[0.2em] mb-4">
                        <Link href="/" className="text-gray-400 hover:text-[#0070f3]">Base</Link>
                        <ChevronRight size={10} className="text-gray-300" />
                        <span className="text-[#0070f3]">Sync Manifest</span>
                    </nav>
                    <div className="flex items-center gap-4">
                        <div className="size-12 bg-[#0070f3] rounded-2xl flex items-center justify-center text-white shadow-lg shadow-blue-500/20">
                            <ShoppingBag size={24} />
                        </div>
                        <h1 className="text-4xl lg:text-6xl font-black text-gray-900 dark:text-white tracking-tighter uppercase">My Cart.</h1>
                    </div>
                </div>
                ...
                {cart.length > 0 ? (
                        ...
                        <div className="flex-1 space-y-6">
                            {cart.map((item) => (
                                <div key={item.id} className="group bg-white dark:bg-white/[0.02] rounded-[3rem] p-8 flex flex-col sm:items-center sm:flex-row gap-8 border border-gray-100 dark:border-white/5 shadow-sm hover:shadow-2xl hover:shadow-blue-500/5 transition-all duration-500">
                                    <div className="relative size-36 bg-gray-50 dark:bg-black rounded-3xl shrink-0 overflow-hidden border border-gray-50 dark:border-white/5">
                                    ...
                                    <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between gap-6 pt-6 sm:pt-0 border-t sm:border-t-0 border-gray-50 dark:border-white/5">
                                        <div className="flex items-center bg-gray-50 dark:bg-white/5 rounded-2xl p-1.5 border border-gray-100 dark:border-white/5">
                                            <button 
                                                ...
                                                className="size-10 flex items-center justify-center hover:bg-white dark:hover:bg-white/10 rounded-xl transition-all text-gray-400 hover:text-red-500"
                                            >
                                            ...
                                            <button 
                                                onClick={() => updateQty(item.id, 1)} 
                                                className="size-10 flex items-center justify-center hover:bg-white dark:hover:bg-white/10 rounded-xl transition-all text-gray-400 hover:text-green-500"
                                            >
                            ...
                            <Link href="/shop/smartphones" className="inline-flex items-center gap-3 text-gray-400 hover:text-[#0070f3] text-[10px] font-black uppercase tracking-[0.3em] mt-12 transition-all group">
                                <ArrowLeft size={16} className="group-hover:-translate-x-2 transition-transform" strokeWidth={3} /> Continue Acquisition
                            </Link>
                        </div>
                        ...
                        <div className="w-full lg:w-[450px]">
                            <div className="bg-[#111111] rounded-[3.5rem] p-10 lg:p-12 text-white shadow-[0_30px_60px_-15px_rgba(0,112,243,0.3)] sticky top-32 border border-white/5">
                                <h2 className="text-xs font-black mb-10 uppercase tracking-[0.3em] text-[#0070f3] flex items-center gap-4">
                                    Final Manifest <span className="h-[1px] flex-1 bg-white/10"></span>
                                </h2>
                                
                                <div className="space-y-6 mb-10 pb-10 border-b border-white/5">
                                    <div className="flex justify-between items-center">
                                        <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Unit Subtotal</span>
                                        <span className="font-black text-xl tracking-tighter">UGX {subtotal.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Logistics (KLA)</span>
                                ...
                                <div className="mt-10 grid grid-cols-2 gap-4">
                                    <div className="flex flex-col gap-3 p-4 bg-white/5 rounded-2xl border border-white/5">
                                        ...
                                        <p className="text-[8px] font-black uppercase tracking-widest text-gray-400">Express Delivery</p>
                                    </div>
                                    <div className="flex flex-col gap-3 p-4 bg-white/5 rounded-2xl border border-white/5">
                                        <ShieldCheck size={18} className="text-[#0070f3]" />
                                        <p className="text-[8px] font-black uppercase tracking-widest text-gray-400">Verified Grade-A</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="text-center py-32 bg-gray-50 dark:bg-white/[0.02] rounded-[4rem] border-2 border-dashed border-gray-100 dark:border-white/5">
                        <div className="size-24 bg-white dark:bg-black rounded-[2.5rem] flex items-center justify-center mx-auto mb-8 shadow-xl border border-gray-100 dark:border-white/5 text-gray-200">
                            <ShoppingBag size={48} strokeWidth={1} />
                        </div>
                        <h2 className="text-3xl font-black text-gray-900 dark:text-white uppercase tracking-tighter mb-3">Manifest Empty.</h2>
                        <p className="text-gray-400 text-xs font-black uppercase tracking-widest mb-10">Waiting for hardware initialization...</p>
